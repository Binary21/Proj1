...
//       implemented.
char UART_getChar(UART* uart_p); // Gets when we know theirs a transmission
bool UART_hasChar(UART* uart_p); // Checking for transmission
bool UART_canSend(UART* uart_p); // Identifies whether the device UART can recieve input from machine
void UART_putChar(UART* uart_p, char c); // sends the character to the serial terminal
void UART_puts(UART* uart_p, char* c);
